...
#         x = self.linear3(x)
#         return x
#         # returns the val for each possible action
...
class ACTrainer:
    def __init__(self, actor_model, critic_model, lr, gamma):
    # def __init__(self, actor_model, critic_model, value_net, lr, gamma):
        self.lr = lr
        self.gamma = gamma
    ...
    def compute_returns(next_value, rewards, dones, gamma = 0.9):
        R = next_value
        returns = []
    ...
    def train_step(self, state, action, reward, next_state, done):
        ...
        next_value = self.critic_model(next)      
        returns = compute
